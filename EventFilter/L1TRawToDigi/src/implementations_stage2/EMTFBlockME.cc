...
namespace l1t {
  namespace stage2 {
	...
	int new_csc_ID = _csc_ID+1; // Shift from 0,1,2... convention to 1,2,3...
	...
	else { int arr[] = {_station, new_csc_ID, _sector, -99, -99}; std::vector<int> vec(arr, arr+5); return vec; }
	...
	else                   { int arr[] = {_station, new_csc_ID, new_sector, -99, -99}; std::vector<int> vec(arr, arr+5); return vec; }
      }
	...
	Hit_.set_sector ( (res->at(iOut)).PtrEventHeader()->Sector() );
	// Hit_.set_chamber();
	// Hit_.set_layer();
	...
	// std::cout << "Got converted values station = " << Hit_.Station() << ", CSC ID = " << Hit_.CSC_ID() 
	// << ", sector = " << Hit_.Sector() << ", subsector = " << Hit_.Subsector() << ", neighbor = " << Hit_.Neighbor() << std::endl;
	
	Hit_.set_sector_GMT ( (res->at(iOut)).PtrEventHeader()->SP_TS() );
	Hit_.set_ring       ( Hit_.calc_ring( Hit_.Station(), Hit_.CSC_ID(), Hit_.Strip() ) );
	// std::cout << "From strip = " << Hit_.Strip() << " got ring = " << Hit_.Ring() << std::endl;
