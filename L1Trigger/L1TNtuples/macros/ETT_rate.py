...
def main():
    ...
    chains['Vtx'] = []  ## Reconstructed vertices
    ...
    for i in range(len(in_file_names)):
        ...
        chains['Evt'].append( TChain('l1EventTree/L1EventTree') )
        chains['Vtx'].append( TChain('l1RecoTree/RecoTree') )
        chains['Unp'].append( TChain('l1UpgradeTree/L1UpgradeTree') )
        chains['Emu'].append( TChain('l1UpgradeEmuTree/L1UpgradeTree') )
    ...
    for iCh in range(len(chains['Unp'])):
        ...
        # chains['Evt'][iCh].SetBranchAddress('Event',     AddressOf(Evt_br))
        chains['Vtx'][iCh].SetBranchAddress('Vertex',    AddressOf(Vtx_br))
        # chains['Unp'][iCh].SetBranchAddress('L1Upgrade', AddressOf(Unp_br))
        # chains['Emu'][iCh].SetBranchAddress('L1Upgrade', AddressOf(Emu_br))


        print '\nEntering loop over events for chain %d' % iCh
        for jEvt in range(chains['Vtx'][iCh].GetEntries()):
            ...
            if iEvt % PRT_EVT is 0: print '  * Run %d, LS %d, event %d, nPV = %d' % (int(Evt_br.run), int(Evt_br.lumi), int(Evt_br.event), int(Vtx_br.nVtx))
        ...
        print 'In chain %d, out of %d events, now %d unpacked, %d emulated events with muons' % (iCh, jEvt, nEvtSingleMu0_unp, nEvtSingleMu0_emu)
        print 'In chain %d, out of %d events, now %d unpacked, %d emulated events with jets (ET > 60)' % (iCh, jEvt, nEvtSingleJet60_unp, nEvtSingleJet60_emu)
        print 'In chain %d, out of %d events, now %d unpacked, %d emulated events with ETT > 500' % (iCh, jEvt, nEvtETT500_unp, nEvtETT500_emu)
    ...
    print '*******   TOTAL RATES   *******'
    print '*******************************'
    print 'Number of events              %6d    %6.1f kHz' % (iEvt+1,                  (iEvt+1)                * ZBRATE / (iEvt+1))
    print 'Unpacked muons                %6d    %6.1f kHz' % (nEvtSingleMu0_unp,       nEvtSingleMu0_unp       * ZBRATE / (iEvt+1))
    print '  * pT >= 22                  %6d    %6.1f kHz' % (nEvtSingleMu22_unp,      nEvtSingleMu22_unp      * ZBRATE / (iEvt+1))
    print 'Emulated muons                %6d    %6.1f kHz' % (nEvtSingleMu0_emu,       nEvtSingleMu0_emu       * ZBRATE / (iEvt+1)) 
    print '  * pT >= 22                  %6d    %6.1f kHz' % (nEvtSingleMu22_emu,      nEvtSingleMu22_emu      * ZBRATE / (iEvt+1))
    print 'Unpacked jets (ET > 60)       %6d    %6.1f kHz' % (nEvtSingleJet60_unp,     nEvtSingleJet60_unp     * ZBRATE / (iEvt+1))
    print '  * ET >= 180                 %6d    %6.1f kHz' % (nEvtSingleJet180_unp,    nEvtSingleJet180_unp    * ZBRATE / (iEvt+1))
    print 'Emulated jets (ET > 60)       %6d    %6.1f kHz' % (nEvtSingleJet60_emu,     nEvtSingleJet60_emu     * ZBRATE / (iEvt+1)) 
    print '  * ET >= 180                 %6d    %6.1f kHz' % (nEvtSingleJet180_emu,    nEvtSingleJet180_emu    * ZBRATE / (iEvt+1))
    print 'Unpacked ETMHF > 120          %6d    %6.1f kHz' % (nEvtETMHF120_unp,        nEvtETMHF120_unp        * ZBRATE / (iEvt+1))
    print 'Emulated ETMHF > 120          %6d    %6.1f kHz' % (nEvtETMHF120_emu,        nEvtETMHF120_emu        * ZBRATE / (iEvt+1))
    print 'Unpacked HTTer > 320          %6d    %6.1f kHz' % (nEvtHTT320_unp,          nEvtHTT320_unp          * ZBRATE / (iEvt+1))
    print 'Emulated HTTer > 320          %6d    %6.1f kHz' % (nEvtHTT320_emu,          nEvtHTT320_emu          * ZBRATE / (iEvt+1))
    print 'Unpacked ETT (ET > 500)       %6d    %6.1f kHz' % (nEvtETT500_unp,          nEvtETT500_unp          * ZBRATE / (iEvt+1))
    print '  * ET >= 1000                %6d    %6.1f kHz' % (nEvtETT1000_unp,         nEvtETT1000_unp         * ZBRATE / (iEvt+1))
    print '  * ET >= 1500                %6d    %6.1f kHz' % (nEvtETT1500_unp,         nEvtETT1500_unp         * ZBRATE / (iEvt+1))
    print '  * ET >= 2000                %6d    %6.1f kHz' % (nEvtETT2000_unp,         nEvtETT2000_unp         * ZBRATE / (iEvt+1))
    print 'Emulated ETT (ET > 500)       %6d    %6.1f kHz' % (nEvtETT500_emu,          nEvtETT500_emu          * ZBRATE / (iEvt+1)) 
    print '  * ET >= 1000                %6d    %6.1f kHz' % (nEvtETT1000_emu,         nEvtETT1000_emu         * ZBRATE / (iEvt+1))
    print '  * ET >= 1500                %6d    %6.1f kHz' % (nEvtETT1500_emu,         nEvtETT1500_emu         * ZBRATE / (iEvt+1))
    print '  * ET >= 2000                %6d    %6.1f kHz' % (nEvtETT2000_emu,         nEvtETT2000_emu         * ZBRATE / (iEvt+1))
    ...
    h_ETM_unp.Scale(ZBRATE / (iEvt+1))
    h_ETM_emu.Scale(ZBRATE / (iEvt+1))
    h_ETM_unp.Write()
    h_ETM_emu.Write()
    ...
    h_HTT_unp.Scale(ZBRATE / (iEvt+1))
    h_HTT_emu.Scale(ZBRATE / (iEvt+1))
    h_HTT_unp.Write()
    h_HTT_emu.Write()
    ...
    h_ETT_unp.Scale(ZBRATE / (iEvt+1))
    h_ETT_emu.Scale(ZBRATE / (iEvt+1))
    h_ETT_unp.Write()
    h_ETT_emu.Write()
    ...
    for i in range(7):
        ...
        h_Njet_emu[i].Scale(ZBRATE / (iEvt+1))
        h_Njet_emu[i].Write()
